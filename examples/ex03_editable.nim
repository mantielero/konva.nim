...
proc newStep*(val:Step):StepObj = 
  ...
  # Step Number
  let stepNumberH = 20
  let stepNumber = newText( Text( width: (group.width().cint), 
                                  text: &"{val.n}", 
  ...
  # Step Title
  let stepTitle = newText( Text( width: group.width().cint, text: val.title, 
  ...
  for actionee in val.by:
    ...
    let actioneeTxt = newText( Text( width: group.width().cint, text: actionee, 
  ...
  if val.height != 0:  # Force step height
    h = val.height.int
  let rect = newRect(Rect( width:group.width().cint, height: h.cdouble, 
...
proc main =
  #var stage:StageObj = newStage("container", 1000, 800)
  var stage = newStage()
